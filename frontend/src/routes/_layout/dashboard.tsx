...
import { AlertTriangle, Clock, Navigation, Cloud, MapPin } from "lucide-react"
...
function IconCircle({ icon, color, bg }: { icon: React.ReactNode; color: string; bg: string }) {
  return (
    <Flex w="32px" h="32px" borderRadius="full" bg={bg} align="center" justify="center" mb={3} style={{ color }}>
      {icon}
    </Flex>
  )
}
    ...
    <Box bg={PAGE_BG} minH="100vh">
      <Container maxW="full" p={6}>
        <Box mb={6}>
          ...
          <Text color={MUTED} fontSize="sm">Real-time cancellation risk, transport alternatives, and cost breakdown · Hyderabad</Text>
        </Box>
        ...
        <VStack gap={5} align="stretch">
          ...
          <Card>
            <Grid templateColumns={{ base: "1fr", md: "1fr 1fr 80px 120px auto auto" }} gap={4} alignItems="end">
              ...
              <Field label="TIME">
                <Input type="time" value={timeStr} onChange={(e) => setTimeStr(e.target.value)} bg={INPUT_BG} borderColor={BORDER} borderRadius="8px" color={PRIMARY} />
              </Field>
          ...
          {!formData ? (
            ...
            <>
              ...
              <Grid templateColumns={{ base: "1fr 1fr", lg: "repeat(4, 1fr)" }} gap={4}>
                <Card topColor={RED}>
                  <IconCircle icon={<AlertTriangle size={16} />} color={RED} bg="#fef2f2" />
                  <CardLabel>Cancellation Risk</CardLabel>
                  {predictionQuery.isLoading ? <Skeleton h="40px" /> : predictionQuery.isError ? <ErrorCard message="Could not load — check backend" onRetry={() => predictionQuery.refetch()} /> : predictionQuery.data ? (
                    <>
                      <Text fontSize="1.8rem" fontWeight="700" color={getRiskTextColor(predictionQuery.data.risk_level)} lineHeight="1" mb={1}>{predictionQuery.data.risk_level.toUpperCase()}</Text>
                      <Text fontSize="0.78rem" color={SUBTLE}>{Math.round(predictionQuery.data.probability * 100)}% probability</Text>
                    </>
                  ) : <Text color={MUTED} fontSize="sm">—</Text>}
                </Card>

                <Card topColor={BLUE}>
                  <IconCircle icon={<Clock size={16} />} color={BLUE} bg="#eff6ff" />
                  <CardLabel>Real Wait Time</CardLabel>
                  {reliabilityQuery.isLoading ? <Skeleton h="40px" /> : reliabilityQuery.isError ? <ErrorCard message="Could not load — check backend" onRetry={() => reliabilityQuery.refetch()} /> : reliabilityQuery.data ? (
                    <>
                      <Text fontSize="1.8rem" fontWeight="700" color={BLUE} lineHeight="1" mb={1}>{reliabilityQuery.data.avg_wait_min} min</Text>
                      <Text fontSize="0.78rem" color={SUBTLE}>avg · {reliabilityQuery.data.label}</Text>
                    </>
                  ) : <Text color={MUTED} fontSize="sm">—</Text>}
                </Card>

                <Card topColor={GREEN}>
                  <IconCircle icon={<Navigation size={16} />} color={GREEN} bg="#f0fdf4" />
                  <CardLabel>Best Mode Now</CardLabel>
                  {alternativesQuery.isLoading ? <Skeleton h="40px" /> : alternativesQuery.isError ? <ErrorCard message="Could not load — check backend" onRetry={() => alternativesQuery.refetch()} /> : bestOption ? (
                    <>
                      <Text fontSize="1.8rem" fontWeight="700" color={TEAL} lineHeight="1" mb={1} textTransform="capitalize">{bestOption.mode}</Text>
                      <Text fontSize="0.78rem" color={GREEN}>{savings != null ? `Saves ₹${savings}` : bestOption.reason.slice(0, 28)}</Text>
                    </>
                  ) : <Text color={MUTED} fontSize="sm">—</Text>}
                </Card>

                <Card topColor={AMBER}>
                  <IconCircle icon={<Cloud size={16} />} color={AMBER} bg="#fffbeb" />
                  <CardLabel>Weather Impact</CardLabel>
                  {weatherQuery.isLoading ? <Skeleton h="40px" /> : weatherQuery.isError ? <ErrorCard message="Could not load weather" onRetry={() => weatherQuery.refetch()} /> : weatherQuery.data ? (
                    <>
                      <Text fontSize="1.8rem" fontWeight="700" color={AMBER} lineHeight="1" mb={1}>
                        {weatherQuery.data.conditions ? weatherQuery.data.conditions.split(" ")[0] : weatherQuery.data.is_raining ? "Raining" : "Clear"}
                      </Text>
                      <Text fontSize="0.78rem" color={SUBTLE}>
                        {weatherQuery.data.temperature_c ? `${weatherQuery.data.temperature_c}°C · ${weatherQuery.data.windspeed_kmh} km/h` : weatherQuery.data.is_raining ? "Rain affecting rides" : "Normal conditions"}
                      </Text>
                    </>
                  ) : <Text color={MUTED} fontSize="sm">—</Text>}
                </Card>
              ...
              <Card>
                {alternativesQuery.isLoading ? <Skeleton h="80px" /> : bestOption ? (
                  <Flex align="center" gap={5} wrap="wrap">
                    <Flex w="56px" h="56px" borderRadius="full" align="center" justify="center" fontSize="2rem" bg={`${modeColor(bestOption.mode)}15`} flexShrink={0}>
                      {modeEmoji(bestOption.mode, bestOption.variant)}
                    </Flex>
                    <Box flex="1" minW="200px">
                      <Flex align="center" gap={3} mb={1} wrap="wrap">
                        <Heading size="md" color={PRIMARY} textTransform="capitalize">{bestOption.mode}</Heading>
                        <Box display="inline-block" px={3} py={0.5} borderRadius="full" bg="#dcfce7" color="#16a34a" fontSize="0.7rem" fontWeight="700" letterSpacing="0.04em" textTransform="uppercase">Recommended</Box>
              ...
              <Grid templateColumns={{ base: "1fr", lg: "repeat(3, 1fr)" }} gap={4}>
                <Card topColor={RED}>
                  ...
                  {predictionQuery.isLoading ? <Skeleton h="140px" /> : predictionQuery.isError ? <ErrorCard message="Could not load data — check backend connection" onRetry={() => predictionQuery.refetch()} /> : predictionQuery.data ? (
                    <>
                      <Flex align="center" gap={4} mb={4}>
                        <Box w="72px" h="72px" borderRadius="full" borderWidth="5px" borderColor={getRiskBorderColor(predictionQuery.data.risk_level)} display="flex" alignItems="center" justifyContent="center">
                          <Text fontWeight="700" fontSize="md" color={getRiskTextColor(predictionQuery.data.risk_level)}>{Math.round(predictionQuery.data.probability * 100)}%</Text>
                        </Box>
                        <Box>
                          <Box display="inline-block" px={3} py={1} borderRadius="full" fontSize="0.7rem" fontWeight="700" letterSpacing="0.05em" textTransform="uppercase" mb={1}
                          ...
                          <Text fontSize="xs" color={MUTED}>{predictionQuery.data.using_ml_model ? "XGBoost ML" : "Rule-based"}</Text>
                        </Box>
                      </Flex>
                      <VStack gap={2} align="stretch">
                        {predictionQuery.data.factors.slice(0, 4).map((f, i) => (
                          <Flex key={i} gap={2} align="center">
                            <Text fontSize="sm">{f.impact === "positive" ? "✅" : f.impact === "negative" ? "⚠️" : "ℹ️"}</Text>
                ...
                <Card topColor={AMBER}>
                  ...
                  {weatherQuery.isLoading ? <Skeleton h="140px" /> : weatherQuery.isError ? <ErrorCard message="Could not load data — check backend connection" onRetry={() => weatherQuery.refetch()} /> : weatherQuery.data ? (
                    <>
                      <Flex align="center" gap={3} mb={3}>
                        <WeatherIcon data={weatherQuery.data} />
                        <Box>
                          <Text fontWeight="700" fontSize="lg" color={PRIMARY}>{weatherQuery.data.conditions}</Text>
                          <Text color={MUTED} fontSize="sm">{weatherQuery.data.temperature_c}°C · {weatherQuery.data.windspeed_kmh} km/h wind</Text>
                        </Box>
                      </Flex>
                      <Box bg={INPUT_BG} borderRadius="md" p={3} mb={3} border={`1px solid ${BORDER}`}>
                        ...
                        <Flex gap={3} fontSize="sm">
                          <Text color={weatherQuery.data.surge_multiplier_cab > 1 ? AMBER : PRIMARY}>🚗 {weatherQuery.data.surge_multiplier_cab}x</Text>
                          <Text color={weatherQuery.data.surge_multiplier_auto > 1 ? AMBER : PRIMARY}>🛺 {weatherQuery.data.surge_multiplier_auto}x</Text>
                          <Text color={weatherQuery.data.surge_multiplier_bike > 1 ? AMBER : PRIMARY}>🛵 {weatherQuery.data.surge_multiplier_bike}x</Text>
                        </Flex>
                      </Box>
                ...
                <Card topColor={BLUE}>
                  ...
                  {reliabilityQuery.isLoading ? <Skeleton h="140px" /> : reliabilityQuery.isError ? <ErrorCard message="Could not load data — check backend connection" onRetry={() => reliabilityQuery.refetch()} /> : reliabilityQuery.data ? (
                    <>
                      <Flex align="end" gap={2} mb={4}>
                        <Text fontSize="4xl" fontWeight="700" lineHeight="1" color={reliabilityQuery.data.score >= 7 ? GREEN : reliabilityQuery.data.score >= 4 ? AMBER : RED}>{reliabilityQuery.data.score}</Text>
                        <Text color={MUTED} mb={1} fontSize="sm">/10 · {reliabilityQuery.data.label}</Text>
                      </Flex>
                      <VStack gap={3} align="stretch">
                        {[
                          { label: "Cancel Rate", value: `${Math.round(reliabilityQuery.data.cancel_rate * 100)}%`, w: reliabilityQuery.data.cancel_rate * 100, color: RED },
                          { label: "Avg Wait", value: `${reliabilityQuery.data.avg_wait_min} min`, w: Math.min(reliabilityQuery.data.avg_wait_min * 5, 100), color: AMBER },
                        ].map((row) => (
                          <Box key={row.label}>
                        ...
                        <Flex justify="space-between" fontSize="sm">
                          ...
                          <Text fontWeight="600" fontSize="xs" color={PRIMARY}>{reliabilityQuery.data.surge_frequency}</Text>
                        </Flex>
                        <Flex gap={2} flexWrap="wrap">
                          {reliabilityQuery.data.recommended_modes.map((m) => (
                            <Box key={m} display="inline-block" px={2} py={0.5} borderRadius="full" bg="#e6fffa" color={TEAL} fontSize="0.7rem" fontWeight="700" textTransform="uppercase" letterSpacing="0.05em">{m}</Box>
                          ))}
                        </Flex>
                      </VStack>
                    </>
              ...
              <Card>
                <Flex justify="space-between" align="center" mb={4}>
                  <Text fontWeight="700" color={sortMode === "cheapest" ? GREEN : MUTED}>💰 Cheapest</Text>
                  <Box position="relative" w="280px" h="10px" borderRadius="full" bg={`linear-gradient(to right, ${GREEN}, ${BLUE})`} cursor="pointer" onClick={() => setSortMode(sortMode === "cheapest" ? "fastest" : "cheapest")}>
                    <Box position="absolute" top="-7px" left={sortMode === "cheapest" ? "0%" : "92%"} w="24px" h="24px" bg="white" border={`4px solid ${BLUE}`} borderRadius="full" transition="all 0.25s ease" boxShadow="md" />
                  </Box>
                  <Text fontWeight="700" color={sortMode === "fastest" ? BLUE : MUTED}>⚡ Fastest</Text>
                </Flex>
                <Text fontSize="sm" color={MUTED} textAlign="center">Showing <strong>{sortMode}</strong> transport options</Text>
              </Card>
              ...
              <Card>
                ...
                {alternativesQuery.isLoading ? (
                  <VStack gap={3}><Skeleton h="64px" /><Skeleton h="64px" /><Skeleton h="64px" /></VStack>
                  ...
                  <VStack gap={0} align="stretch">
                    {(() => {
                      ...
                      return sorted.map((opt, i) => {
                        ...
                        return (
                          <Box key={i} px={3} py={3} borderBottom={isLast ? "none" : `1px solid ${INPUT_BG}`} borderLeft={isBest ? "4px solid #16a34a" : "4px solid transparent"} bg={isBest ? "#f0fdf4" : "transparent"} opacity={isUnavailable ? 0.45 : 1} _hover={{ bg: isUnavailable ? "transparent" : INPUT_BG }} transition="background 0.15s ease">
                            <Flex align="center" gap={4}>
                              <Flex w="44px" h="44px" borderRadius="full" bg={`${color}18`} align="center" justify="center" fontSize="1.3rem" flexShrink={0} border={`1.5px solid ${color}30`}>
                              ...
                              <Box flex="1">
                                <Flex align="center" gap={2} mb={0.5} flexWrap="wrap">
                                  ...
                                  {isBest && <Box display="inline-block" px={2} py={0.5} borderRadius="full" bg="#dcfce7" color="#16a34a" fontSize="0.62rem" fontWeight="700" textTransform="uppercase" letterSpacing="0.04em">RECOMMENDED</Box>}
                                  {isUnavailable && <Box display="inline-block" px={2} py={0.5} borderRadius="full" bg="#f1f5f9" color={MUTED} fontSize="0.62rem" fontWeight="700" textTransform="uppercase" letterSpacing="0.04em">Not Available</Box>}
                                  {isHighRisk && <Box display="inline-block" px={2} py={0.5} borderRadius="full" bg="#fee2e2" color={RED} fontSize="0.62rem" fontWeight="700" textTransform="uppercase" letterSpacing="0.04em">High Risk</Box>}
                                  {opt.vehicles_needed > 1 && !isUnavailable && <Badge colorPalette="orange" size="sm">{opt.vehicles_needed} vehicles</Badge>}
                                </Flex>
